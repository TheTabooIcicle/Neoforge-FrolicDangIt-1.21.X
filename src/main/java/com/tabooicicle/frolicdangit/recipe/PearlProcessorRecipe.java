...
public record PearlProcessorRecipe(
        ...
        ItemStack outputSlot3, ItemStack outputSlot4, ItemStack outputSlot5) implements Recipe<PearlProcessorRecipeInput> // constructor has slot counts as parameters to keep track
{ // each slot is in the record parameter
    ...
    //    @Override
//    public boolean matches(PearlProcessorRecipeInput pearlProcessorRecipeInput, Level level) {
//        if (level.isClientSide()) {
//            return false;
//        }
//        // check fuel, bottles, and component
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(0), slot0, slot0Count)) return false;
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(1), slot1, slot1Count)) return false;
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(2), slot2, slot2Count)) return false;
//
//        // check items to upgrade
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(3), slot3, slot3Count)) return false;
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(4), slot4, slot4Count)) return false;
//        if (!matchesWithCount(pearlProcessorRecipeInput.getItem(5), slot5, slot5Count)) return false;
//
//        return true;
//    }
    public boolean matches(PearlProcessorRecipeInput input, Level level) {
        if (level.isClientSide()) {
            return false;
        }
// for debugging
...
//        System.out.println("Slot 5 failed");
    ...
    @Override
    public @NotNull ItemStack assemble(PearlProcessorRecipeInput pearlProcessorRecipeInput, HolderLookup.Provider provider) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.PEARL_PROCESSOR_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRecipes.PEARL_PROCESSOR_TYPE.get();
    }

    public static class Serializer implements RecipeSerializer<PearlProcessorRecipe> {
        public static final MapCodec<PearlProcessorRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group( // change codec to read multiple ingredients and each ingredients counts
                Ingredient.CODEC_NONEMPTY.fieldOf("slot0").forGetter(PearlProcessorRecipe::slot0),
                Codec.INT.fieldOf("slot0_count").orElse(1).forGetter(PearlProcessorRecipe::slot0Count),
                Ingredient.CODEC_NONEMPTY.fieldOf("slot1").forGetter(PearlProcessorRecipe::slot1),
                Codec.INT.fieldOf("slot1_count").orElse(1).forGetter(PearlProcessorRecipe::slot1Count),
                Ingredient.CODEC_NONEMPTY.fieldOf("slot2").forGetter(PearlProcessorRecipe::slot2),
                Codec.INT.fieldOf("slot2_count").orElse(1).forGetter(PearlProcessorRecipe::slot2Count),
                Ingredient.CODEC_NONEMPTY.fieldOf("slot3").forGetter(PearlProcessorRecipe::slot3),
                Codec.INT.fieldOf("slot3_count").orElse(1).forGetter(PearlProcessorRecipe::slot3Count),
                Ingredient.CODEC_NONEMPTY.fieldOf("slot4").forGetter(PearlProcessorRecipe::slot4),
                Codec.INT.fieldOf("slot4_count").orElse(1).forGetter(PearlProcessorRecipe::slot4Count),
                Ingredient.CODEC_NONEMPTY.fieldOf("slot5").forGetter(PearlProcessorRecipe::slot5),
                Codec.INT.fieldOf("slot5_count").orElse(1).forGetter(PearlProcessorRecipe::slot5Count),
                // upgraded item outputs
                ItemStack.CODEC.fieldOf("output_slot3").forGetter(PearlProcessorRecipe::outputSlot3),
                ItemStack.CODEC.fieldOf("output_slot4").forGetter(PearlProcessorRecipe::outputSlot4),
                ItemStack.CODEC.fieldOf("output_slot5").forGetter(PearlProcessorRecipe::outputSlot5)
        ).apply(inst, PearlProcessorRecipe::new));

        public static final StreamCodec<RegistryFriendlyByteBuf, PearlProcessorRecipe> STREAM_CODEC =
                StreamCodec.of(
                        (buf, recipe) -> {
                        ...
                        (buf) -> {
                            ...
                            return new PearlProcessorRecipe(slot0, slot0Count, slot1, slot1Count,
        ...
        @Override
        public MapCodec<PearlProcessorRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, PearlProcessorRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}
